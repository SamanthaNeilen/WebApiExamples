...
namespace NetCore.WebApiExample.MiddleWare.Swagger
{
    public class SwaggerDocumentFilter : IDocumentFilter
    {
        ...
        private IDictionary<string, PathItem> GetSortedPaths(SwaggerDocument swaggerDoc)
        {
            return swaggerDoc.Paths.OrderBy(pair => pair.Key).ToDictionary(pair => pair.Key, pair => pair.Value);
        }
    }
}
